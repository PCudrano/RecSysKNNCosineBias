...
class MatrixFactorization_Cython(Recommender, Incremental_Training_Early_Stopping):
    ...
    def _compute_score_MF(self, user_id_array, items_to_compute = None):
        ...
        if items_to_compute is not None:
            item_scores = - np.ones((len(user_id_array), self.H.shape[0]), dtype=np.float32)*np.inf
            item_scores[:, items_to_compute] = np.dot(self.W[user_id_array], self.H[items_to_compute,:].T)
        else:
