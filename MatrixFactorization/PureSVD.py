...
class PureSVDRecommender(Recommender):
    ...
    def _compute_score_SVD(self, user_id_array, items_to_compute = None):
        ...
        if items_to_compute is not None:
            item_scores = - np.ones((len(user_id_array), self.s_Vt.shape[1]), dtype=np.float32)*np.inf
            item_scores[:,items_to_compute] = self.U[user_id_array, :].dot(self.s_Vt[:,items_to_compute])
        else:
